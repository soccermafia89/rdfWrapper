...
public class Reducer {
    ...
    public static DataModel reduce(Model metaData, HashMap<String, Object> data) {
        ...
        //First reduce based on the isLinkedWith property.
        ...
        //Next reduce based on the hash resources
        Map<Object, Resource> uniqueHashes = new HashMap<Object, Resource>();
        
        it = reducedModel.listStatements((Resource) null, RDFSerializerWrapper.hasHash, (Resource) RDFSerializerWrapper.hash);
        while (it.hasNext()) {
            Statement statement = it.nextStatement();

            Resource source = statement.getSubject();
            Object hash = reducedData.get(source.getURI());
            if(uniqueHashes.containsKey(hash)) {
                Resource target = uniqueHashes.get(hash);
                
                Reducer.replace(source, target, reducedModel, reducedData);
            } else {
                uniqueHashes.put(hash, source);
            }
        }
